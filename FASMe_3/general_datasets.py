...
FAKE_NAME = 'df'
TASKS = [
    # 'mis-classified_4+13fmt_all_fixed',
    'new_df_faces_hard'
]
in_root = "/datasets/datasets"
in_path = "/datasets/datasets"
...
def convert_images(task):
    ...
    for src_path in full_file_paths:
        ...
        if '/fake' in src_path:
            if 'makeup' in src_path:
                label = 3
            elif '/pad/' in src_path:
                label = 1
            else:
                label = 2
        ...
        elif os.path.isdir(src_path) is False and (fname.endswith('.mp4') or fname.endswith('.avi') or fname.endswith('.3gp') or ext.endswith('.mov')):
