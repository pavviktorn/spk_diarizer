...
def performances_val_0(map_score_val_filename):
    with open(map_score_val_filename, 'r') as file:
        lines = file.readlines()
    val_scores = []
    val_labels = []
    ...
    type1 = len([s for s in data if s['map_score'] < mythresh and s['label'] == 1])
    type2 = len([s for s in data if s['map_score'] > mythresh and s['label'] == 0])
